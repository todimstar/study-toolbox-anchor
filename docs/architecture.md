...
- 图片优先走 Android Photo Picker / 系统相册选择器，避免默认文件管理器缩略图过小。
- 原生 M4A 录音桥：网页按住说话时优先调用 App 录音并上传 `audio/mp4`，浏览器端再回退到 `MediaRecorder`。
- `MicroChatPollWorker`：Android WorkManager 每 15 分钟左右轮询微聊家长消息，并用独立高优先级微聊通知通道触发系统通知和支持通知点的桌面红点。
...
- 微聊首次加载必须取最新一页消息；加载更早消息时必须保留首个可见消息锚点，避免历史插入后跳到最旧位置。
